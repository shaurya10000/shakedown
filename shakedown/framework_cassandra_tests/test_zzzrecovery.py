...
def test_node_replace_replaces_node():
    ...
    #mds_utils.test_if_node_launched_with_replace(pod_host)
    ...
    # We have only fixed nodes launched with a particular mds_role, testing that the new task comes up on the
    # same node but with params containing -Dcassandra.replace_address
...
def test_shutdown_host():
    ...
    print("Killing host: " + replace_pod['host'])
    try:
        post_api(replace_pod['host'])
    except Exception as e:
        print(str(e))

    print("Killed host: " + replace_pod['host'])
